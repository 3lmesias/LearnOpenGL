...
unsigned int TextureFromFile(const char* imagePath, bool walpha) {
...
int main() {
	...
	unsigned int cubeTexture = TextureFromFile(std::string("Images\\marble.jpg").c_str(),false);
	unsigned int floorTexture = TextureFromFile(std::string("Images\\metal.png").c_str(),false);
	unsigned int grassTexture = TextureFromFile(std::string("Images\\grass.png").c_str(),true);

	unsigned int windowTexture = TextureFromFile(std::string("Images\\window.png").c_str(), true);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//constantShader.use();
		//constantShader.setMat4("model", glm::mat4(1.0f));
		glActiveTexture(GL_TEXTURE0);
