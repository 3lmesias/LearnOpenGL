...
void Model::Draw(Shader2& shader) {
...
unsigned int TextureFromFile(const char* imagePath, std::string  &directory) {
	...
	int height;
	int chanels;
	data = stbi_load(filename.c_str(), &width, &height, &chanels, 0);
	if (data) {
		GLenum format;
...
std::vector<Texture> Model::LoadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName) {
	...
	for (unsigned int i = 0; i < mat->GetTextureCount(type); i++) {
		...
		if (!skip) {
			Texture texture;
			const char* imagePath = str.C_Str();
			texture.id = TextureFromFile(imagePath, directory);
			texture.type = type;
